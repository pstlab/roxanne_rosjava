...
import com.github.roxanne_rosjava.roxanne_rosjava_core.ai.framework.domain.knowledge.ex.HierarchyCycleException;
					...
					if (!master.equals(slave)) 
					{
						try 
						{
							...
							if (!this.dg.containsKey(slave)) {
								this.dg.put(slave, new HashSet<DomainComponent>());
							}
							
							// check if master exists in the DG
							if (!this.dg.containsKey(master)) {
								this.dg.put(master, new HashSet<DomainComponent>());
							}
	...
	private void computeDecompositionGraph()
	{
		...
		for (SynchronizationRule rule : this.pdb.getSynchronizationRules()) 
		{
			...
			if (!this.graph.containsKey(reference)) {
				...
				this.graph.put(reference, new ArrayList<List<ComponentValue>>());
			}
