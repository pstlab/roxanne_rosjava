...
//	}
	...
	public List<ComponentValue> getDirectSuccessors(ComponentValue value) {
		...
		return new ArrayList<ComponentValue>(this.transitions.get(value).keySet());
	}
		...
		if (source.equals(target))
		{
			...
			for (ComponentValue successor : this.getDirectSuccessors(source))
			{
				...
				if (!source.equals(successor)) {
					// get list of paths
					for (ValuePath path : this.computePaths(successor, target, new ArrayList<ComponentValue>())) {
			...
			result = this.computePaths(source, target, new ArrayList<ComponentValue>());
		}
